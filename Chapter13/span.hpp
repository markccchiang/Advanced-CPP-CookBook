...
struct span_storage<E, dynamic_extent> {
    ...
    constexpr span_storage(E* ptr, std::size_t size) noexcept : ptr(ptr), size(size) {}
...
public:
    ...
    TCB_SPAN_CONSTEXPR11 span(pointer ptr, index_type count) : storage_(ptr, count) {
    ...
    TCB_SPAN_CONSTEXPR11 span<element_type, dynamic_extent> first(index_type count) const {
        TCB_SPAN_EXPECT(count <= size());
        return {data(), count};
    }

    TCB_SPAN_CONSTEXPR11 span<element_type, dynamic_extent> last(index_type count) const {
        TCB_SPAN_EXPECT(count <= size());
        return {data() + (size() - count), count};
    }

    TCB_SPAN_CONSTEXPR11 span<element_type, dynamic_extent> subspan(index_type offset, index_type count = dynamic_extent) const {
        TCB_SPAN_EXPECT(offset <= size() && (count == dynamic_extent || offset + count <= size()));
